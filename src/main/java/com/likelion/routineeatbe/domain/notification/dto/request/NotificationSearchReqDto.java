package com.likelion.routineeatbe.domain.notification.dto.request;
...
public record NotificationSearchReqDto(
        @Schema(description = "사용자 고유 식별번호", example = "1234", requiredMode = Schema.RequiredMode.REQUIRED)
        ...
        @Schema(description = "1부터 시작하는 조회 위치", example = "1", defaultValue = "1", minimum = "1")
        @Min(value = 1, message = "커서는 1 이상이어야 합니다.")
        @Max(value = 2147483547, message = "커서는 2147483547 이하여야 합니다.")
        Integer cursor,

        @Schema(description = "한 번에 조회할 알림 개수", example = "10", defaultValue = "10", minimum = "1", maximum = "100")
        @Min(value = 1, message = "조회 크기는 1 이상이어야 합니다.")
        @Max(value = 100, message = "조회 크기는 100 이하여야 합니다.")
        Integer size
) {

    public NotificationSearchReqDto {
