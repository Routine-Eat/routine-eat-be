...
class MenuControllerTest {
    ...
    @Mock
    private InitMenuDifficultyLevelService initMenuDifficultyLevelService;
    ...
    @Test
    @DisplayName("메뉴 난이도 초기화 API가 201과 초기화 개수를 반환한다")
    void 메뉴_난이도_초기화_API_201_반환_성공() {
        // given
        given(initMenuDifficultyLevelService.initialize())
                .willReturn(InitMenuDifficultyLevelResDto.create(1156L));

        // when
        ResponseEntity<GlobalResponse<InitMenuDifficultyLevelResDto>> response =
                menuController.initMenuDifficultyLevels();
